...
const Features: React.FC = () => {
  ...
  return (
    <section
      id="features"
      className={`py-24 scroll-mt-20 transition-colors duration-300 ${isLight ? "bg-white" : "bg-[#0a0a0a]"}`}
    >
            ...
            <div key={i} className="text-center group">
              <div className="mb-6 flex justify-center transform transition-transform duration-300 group-hover:-translate-y-2">
                {React.cloneElement(f.icon as React.ReactElement, {
                  className: `w-8 h-8 ${isLight ? "text-[#d4af37]" : "gold-text"}`,
                })}
              </div>
              <h4
                className={`text-lg font-serif font-bold mb-4 tracking-wide uppercase ${isLight ? "text-black" : "text-white"}`}
              >
                {f.title}
              </h4>
              <p
                className={`text-sm font-light leading-relaxed ${isLight ? "text-zinc-700" : "text-zinc-400"}`}
              >
                {f.desc}
              </p>
            </div>
